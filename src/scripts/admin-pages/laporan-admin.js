...
const createLaporanAdmin = () => {
  ...
  logo.textContent = 'ADMIN SIPRASA';
...
// Function to fetch all laporan data (for PDF export)
const fetchLaporanDataAll = async () => {
  const userId = localStorage.getItem('userId');
  const authToken = localStorage.getItem('authToken');
...
const printReport = async () => {
  ...
  // Fetch all data
  const laporanData = await fetchLaporanDataAll();
  ...
  const tableHeaders = ['No', 'Judul', 'Status', 'Kategori', 'Lokasi', 'Tanggal'];

  // Prepare table rows
  const tableRows = laporanData.map((laporan, index) => [
    index + 1, // Nomor urut
    laporan.judul, // Judul laporan
    laporan.status, // Status
    laporan.kategori, // Kategori
    laporan.lokasi, // Lokasi
    laporan.tanggal, // Tanggal laporan
  ]);
  ...
  // Initialize Y position for autoTable
  let startY = 30;

  // Split rows into chunks of 15
  const chunkSize = 15;
  for (let i = 0; i < tableRows.length; i += chunkSize) {
    ...
    doc.autoTable({
      ...
      didDrawPage: function (data) {
        ...
        if (i + chunkSize < tableRows.length) {
          doc.addPage();
        }
      },
    });
